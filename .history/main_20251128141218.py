...
def main():
	...
	with st.sidebar:
		...
		randomize_seed = st.checkbox('Randomize returns each run', value=True)
	...
	if st.button('Run simulation'):
		...
		sim_years = int(years)
		seed_value = int(random_seed_input) if not randomize_seed else np.random.default_rng().integers(0, 2**32 - 1)

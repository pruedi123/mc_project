...
def _simulate_draw(payload: Dict[str, Any]) -> Dict[str, float]:
	rng = np.random.default_rng(int(payload['seed']))
	taxable_series = sample_lognormal_returns(payload['years'], payload['taxable_log_drift'], payload['taxable_log_volatility'], rng)
	roth_series = sample_lognormal_returns(payload['years'], payload['roth_log_drift'], payload['roth_log_volatility'], rng)
	df_run = simulate_withdrawals(
		start_age_primary=payload['start_age'],
		start_age_spouse=payload['start_age_spouse'],
		years=payload['years'],
		taxable_start=payload['taxable_start'],
		stock_total_return=payload['stock_total_return'],
		stock_dividend_yield=payload['stock_dividend_yield'],
		stock_turnover=payload['stock_turnover'],
		bond_return=payload['bond_return'],
		roth_start=payload['roth_start'],
		tda_start=payload['tda_start'],
		tda_spouse_start=payload['tda_spouse_start'],
		withdraw_amount=payload['withdraw_amount'],
		target_stock_pct=payload['target_stock_pct'],
		taxable_stock_basis_pct=payload['taxable_stock_basis_pct'],
		taxable_bond_basis_pct=payload['taxable_bond_basis_pct'],
		gross_up_withdrawals=payload['gross_up_withdrawals'],
		rmd_start_age=payload['rmd_start_age'],
		rmd_start_age_spouse=payload['rmd_start_age_spouse'],
		ss_income_annual=payload['ss_income_annual'],
		ss_income_spouse_annual=payload['ss_income_spouse_annual'],
		ss_cola=payload['ss_cola'],
		pension_income_annual=payload['pension_income_annual'],
		pension_income_spouse_annual=payload['pension_income_spouse_annual'],
		pension_cola=payload['pension_cola'],
		other_income_annual=payload['other_income_annual'],
		filing_status=payload['filing_status'],
		use_itemized_deductions=payload['use_itemized_deductions'],
		itemized_deduction_amount=payload['itemized_deduction_amount'],
		roth_conversion_amount=payload['roth_conversion_amount'],
		roth_conversion_years=payload['roth_conversion_years'],
		roth_conversion_tax_source=payload['roth_conversion_tax_source'],
		stock_return_series=taxable_series,
		bond_return_series=taxable_series,
		roth_return_series=roth_series,
		life_expectancy_primary=payload['life_expectancy_primary'],
		life_expectancy_spouse=payload['life_expectancy_spouse']
	)
	return compute_summary_metrics(df_run, payload['inheritor_rate'])


@st.cache_data
def _cached_monte_carlo(num_runs: int,
						...
						years: int,
						seed: Optional[int],
						num_jobs: int) -> List[Dict[str, float]]:
	if num_runs <= 0 or years <= 0:
		return []
	base_rng = np.random.default_rng(seed)
	seeds = base_rng.integers(0, 2 ** 32 - 1, size=num_runs)
	payloads = [{
		'seed': int(int_seed),
		'years': years,
		...
		'roth_log_drift': roth_log_drift,
		'roth_log_volatility': roth_log_volatility,
	...
	} for int_seed in seeds]
	results = []
	if num_jobs < 2 or num_runs < 200:
		for payload in payloads:
			results.append(_simulate_draw(payload))
	else:
		with ProcessPoolExecutor(max_workers=num_jobs) as executor:
			results = list(executor.map(_simulate_draw, payloads))
	return results
...
def run_monte_carlo(num_runs: int,
					...
					num_jobs: int = 4) -> List[Dict[str, float]]:
	return _cached_monte_carlo(
		num_runs, withdraw_amount, start_age, start_age_spouse, taxable_start,
		stock_total_return, stock_dividend_yield, stock_turnover, bond_return,
		roth_start, tda_start, tda_spouse_start, target_stock_pct,
		taxable_stock_basis_pct, taxable_bond_basis_pct, gross_up_withdrawals,
		rmd_start_age, rmd_start_age_spouse, ss_income_annual, ss_income_spouse_annual,
		ss_cola, pension_income_annual, pension_income_spouse_annual, pension_cola,
		other_income_annual, filing_status, use_itemized_deductions,
		itemized_deduction_amount, roth_conversion_amount, roth_conversion_years,
		roth_conversion_tax_source, taxable_log_drift, taxable_log_volatility,
		roth_log_drift, roth_log_volatility, life_expectancy_primary,
		life_expectancy_spouse, inheritor_rate, years, seed, num_jobs
	)
...
def evaluate_success_rate(withdraw_amount: float,
						  ...
						  num_jobs: int) -> Tuple[float, pd.DataFrame]:
	if years <= 0:
		return 1.0, pd.DataFrame()
	key = (int(start_total), years, round(withdraw_amount, 2))
	if key in cache:
...
def search_withdrawal_for_target(start_total: float,
	...
	best_withdrawal = 0.0
	best_success = 0.0
	success_high, last_df = evaluate_success_rate(high, years, mc_kwargs, mc_runs, seed_provider, start_total, cache, num_jobs)
	doubling = 0
	...
	for _ in range(max_iterations):
		mid = (low + high) / 2.0
		success_mid, mid_df = evaluate_success_rate(mid, years, mc_kwargs, mc_runs, seed_provider, start_total, cache, num_jobs)
...
def build_guardrail_ratios(total_years: int,
						  mc_kwargs: Dict[str, Any],
						  mc_runs: int,
						  tolerance: float,
						  max_iterations: int,
						  seed_provider,
						  num_jobs: int,
						  start_total: float) -> Dict[int, Dict[str, float]]:
	"""Return guardrail ratios per horizon using the withdrawal finder for 90/85/75% success."""
	target_successes = (0.90, 0.85, 0.75)
	guardrails: Dict[int, Dict[str, float]] = {}
	for years in range(total_years, 0, -1):
		per_year_kwargs = dict(mc_kwargs)
		per_year_kwargs['years'] = years
		ratio_map: Dict[float, float] = {}
		for target_success in target_successes:
			seed = seed_provider()
			withdrawal, _, _ = find_withdrawal_for_success(
				...
				seed=seed,
				mc_kwargs=per_year_kwargs
			)
			ratio_map[target_success] = withdrawal / max(start_total, 1.0)
		ratio90 = ratio_map.get(0.90, 0.0)
		ratio85 = max(ratio_map.get(0.85, 0.0), ratio90)
		ratio75 = max(ratio_map.get(0.75, 0.0), ratio85)
		guardrails[years] = {'ratio90': ratio90, 'ratio85': ratio85, 'ratio75': ratio75}
	return guardrails
...
def run_closed_loop_monte_carlo(initial_withdrawal: float,
	...
	last_success = 0.0
	start_total_base = sum_account_values(base_mc_kwargs)
	guardrail_ratios = build_guardrail_ratios(
		total_years=total_years,
		mc_kwargs=dict(base_mc_kwargs),
		mc_runs=mc_runs,
		tolerance=tolerance,
		max_iterations=max_iterations,
		seed_provider=seed_provider,
		num_jobs=num_jobs,
		start_total=start_total_base
	)
	...
	for year in range(1, total_years + 1):
		...
		guardrail_to_display = {}
		if remaining_years > 0:
			start_total = sum_account_values(kwargs)
			current_ratio = current_withdrawal / start_total if start_total > 0 else float('inf')
			guardrail = guardrail_ratios.get(remaining_years, {})
			ratio_90 = guardrail.get('ratio90', 0.0)
			ratio_85 = guardrail.get('ratio85', 0.0)
			ratio_75 = guardrail.get('ratio75', 0.0)
			guardrail_to_display = {
				'guardrail_ratio_90': ratio_90,
				'guardrail_ratio_85': ratio_85,
				'guardrail_ratio_75': ratio_75,
				'current_ratio': current_ratio if start_total > 0 else None
			}
			success_rate = 0.85
			if ratio_85 > 0 and start_total > 0:
				if current_ratio <= ratio_90:
					success_rate = 0.90
					new_withdrawal = min(start_total * ratio_85, max_permitted)
					if new_withdrawal > current_withdrawal + 1e-6:
						current_withdrawal = new_withdrawal
						adjustment_note = '↑ to 85%'
				elif current_ratio >= ratio_75:
					success_rate = 0.75
					new_withdrawal = min(start_total * ratio_85, max_permitted)
					if new_withdrawal + 1e-6 < current_withdrawal:
						current_withdrawal = new_withdrawal
						adjustment_note = '↓ to 85%'
				else:
					success_rate = 0.85
			...
			guardrail_to_display = {
				'guardrail_ratio_90': None,
				'guardrail_ratio_85': None,
				'guardrail_ratio_75': None,
				'current_ratio': None
			}
		results.append({
			'year': year,
		...
		results[-1].update(guardrail_to_display)
	return {
		'records': results,
...
def main():
	...
	finder_target_success_pct = st.number_input('Target success rate (%)', value=75, min_value=50, max_value=99, step=1)
	...
	hist_source_closed = st.selectbox('Historical source for the closed-loop path', list(HISTORICAL_SOURCES.keys()), index=1, key='closed_loop_source')
	hist_schedule_preview = load_historical_schedule(hist_source_closed)
	available_rows = len(hist_schedule_preview)
	max_closed_loop_start = max(0, available_rows - int(years) * 12)
	if available_rows < years * 12:
		st.warning('Not enough historical months to cover the full horizon; the path will stop when data ends.')
	...
	closed_loop_runs = st.number_input('Closed-loop Monte Carlo runs per iteration', min_value=100, max_value=5000, value=500, step=100)
	closed_loop_tolerance = st.number_input('Withdrawal tolerance ($)', min_value=10.0, value=1000.0, step=50.0, format="%.2f")
	...
	try:
		max_closed_loop_workers = multiprocessing.cpu_count()
	except NotImplementedError:
		max_closed_loop_workers = 4
	max_closed_loop_workers = max(1, max_closed_loop_workers)
	closed_loop_jobs = st.number_input(
		'Parallel workers for closed-loop Monte Carlo',
		min_value=1,
		max_value=max_closed_loop_workers,
		value=min(4, max_closed_loop_workers),
		step=1,
		key='closed_loop_jobs'
	)
	if st.button('Run closed-loop Monte Carlo'):
		with st.spinner('Running dynamic withdrawal strategy...'):
			hist_schedule = hist_schedule_preview
	...
	if 'closed_loop_results' in st.session_state:
		...
		if cl_res.get('records'):
			...
			percent_or_blank = lambda x: '' if x is None or (isinstance(x, float) and np.isinf(x)) else f"{x:.2%}"
			st.dataframe(closed_df.style.format({
				...
				'total_taxes': '{:,.0f}'.format,
				'guardrail_ratio_90': percent_or_blank,
				'guardrail_ratio_85': percent_or_blank,
				'guardrail_ratio_75': percent_or_blank,
				'current_ratio': percent_or_blank
			}))

...
def run_monte_carlo(num_runs: int,
	...
	for run_idx in range(num_runs):
		...
		bond_series = sample_bond_returns(years, bond_factors, rng)
...
def simulate_withdrawals(start_age_primary: int,
	...
	for y in range(1, years+1):
		...
		# Tax attributable to non-spending income (reinvested items):
		# - RMD excess reinvested in taxable (ordinary income at marginal rate)
		# - Bond interest reinvested (ordinary income at marginal rate)
		# - Dividends reinvested (cap gains rate)
		# - Turnover gains (cap gains rate, paper round-trip)
		# These taxes are a portfolio drag, not a spending reduction.
		non_spending_tax = (
			rmd_excess_to_taxable * marginal_ordinary_rate
			+ max(0.0, interest) * marginal_ordinary_rate
			+ (div + turnover_realized_gain) * marginal_cg_rate
		)
		non_spending_tax = max(0.0, min(non_spending_tax, total_taxes - roth_conversion_tax_delta))
		...
		rows.append({
			...
			'withdrawal_used': wanted,
			'after_tax_spending': (out_taxable_cash + withdraw_from_tda + withdraw_from_roth
				- rmd_excess_to_taxable + ss_income + pension_income + other_income
				- (total_taxes - roth_conversion_tax_delta - non_spending_tax)),
		})
...
def main():
	...
	with st.sidebar:
		...
		if return_mode == 'Simulated (lognormal)':
			st.caption('Stocks use lognormal draws; bonds sampled from historical LBM 100 F.')
			taxable_log_drift = st.number_input('Stock log drift (µ)', value=0.07, format="%.4f")
			taxable_log_volatility = st.number_input('Stock log volatility (σ)', value=0.127, format="%.4f")
		...
		st.markdown('---')
		st.markdown('**Dynamic withdrawal guardrails**')
	...
	if guardrails_enabled:
		...
		if return_mode == 'Simulated (lognormal)':
			...
			# Estimate stock params from historical data
			mg_df = load_master_global()
		...
		if return_mode == 'Historical (master_global_factors)' and historical_mode_type == 'All periods (distribution)':
			...
			with st.spinner(spinner_msg):
				mc_results, all_yearly_df = run_monte_carlo(
					...
					bond_factors=load_bond_factors(),
					years=int(years),
					guardrails_enabled=bool(guardrails_enabled),
					guardrail_lower=float(guardrail_lower),
					guardrail_upper=float(guardrail_upper),
	...
	if sim_mode in ('simulated', 'historical_dist') and 'mc_percentile_rows' in st.session_state:
		...
		port_pcts = all_yearly.groupby('year')['total_portfolio'].quantile([0.00, 0.10, 0.25, 0.50, 0.75, 0.90]).unstack()
		port_pcts.columns = ['0th (min)', '10th', '25th', '50th (median)', '75th', '90th']
		st.line_chart(port_pcts)
